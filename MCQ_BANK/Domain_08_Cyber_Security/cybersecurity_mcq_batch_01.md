# Cyber Security MCQ Bank - Batch 01

## Questions 1-100

---

### Question 1
Domain: Cyber Security
Topic: Introduction to Cyber Security
Subtopic: CIA Triad
Difficulty: Easy

Question: What does CIA stand for in information security?
A) Central Intelligence Agency
B) Confidentiality, Integrity, Availability
C) Computer Internet Access
D) Cyber Information Analysis

✔ Correct Answer: B
Reason: CIA Triad represents three core principles of information security.
Tag: Past Paper

---

### Question 2
Domain: Cyber Security
Topic: Security Fundamentals & Principles
Subtopic: Defense in Depth
Difficulty: Medium

Question: What is defense in depth?
A) Single security layer
B) Multiple layers of security controls
C) No security
D) Physical security only

✔ Correct Answer: B
Reason: Defense in depth uses multiple security layers to protect assets.
Tag: Normal

---

### Question 3
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: Encryption Types
Difficulty: Easy

Question: What is symmetric encryption?
A) Different keys for encryption and decryption
B) Same key for encryption and decryption
C) No key needed
D) Public key only

✔ Correct Answer: B
Reason: Symmetric encryption uses single shared key for both operations.
Tag: Past Paper

---

### Question 4
Domain: Cyber Security
Topic: Network Security
Subtopic: Firewall
Difficulty: Easy

Question: What is a firewall?
A) Physical wall
B) Network security device filtering traffic
C) Antivirus software
D) Backup system

✔ Correct Answer: B
Reason: Firewall monitors and controls incoming/outgoing network traffic.
Tag: Normal

---

### Question 5
Domain: Cyber Security
Topic: Operating System Security
Subtopic: Access Control
Difficulty: Easy

Question: What is the principle of least privilege?
A) Maximum access for all users
B) Minimum access rights necessary for tasks
C) No access control
D) Random access

✔ Correct Answer: B
Reason: Least privilege grants only essential permissions to minimize risk.
Tag: Past Paper

---

### Question 6
Domain: Cyber Security
Topic: Web Application Security
Subtopic: SQL Injection
Difficulty: Medium

Question: What is SQL injection?
A) Database backup
B) Malicious SQL code insertion
C) SQL optimization
D) Database encryption

✔ Correct Answer: B
Reason: SQL injection exploits vulnerabilities by injecting malicious SQL queries.
Tag: Normal

---

### Question 7
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: Malware Types
Difficulty: Easy

Question: What is a virus?
A) Hardware failure
B) Self-replicating malicious code
C) Network protocol
D) Encryption method

✔ Correct Answer: B
Reason: Virus is malware that replicates by attaching to other programs.
Tag: Normal

---

### Question 8
Domain: Cyber Security
Topic: Authentication & Access Control
Subtopic: Multi-Factor Authentication
Difficulty: Easy

Question: What is two-factor authentication (2FA)?
A) Single password
B) Two different authentication methods
C) Two passwords
D) No authentication

✔ Correct Answer: B
Reason: 2FA requires two independent credentials for verification.
Tag: Normal

---

### Question 9
Domain: Cyber Security
Topic: Secure Software Development
Subtopic: SDLC Security
Difficulty: Medium

Question: What is secure SDLC?
A) Fast development
B) Integrating security throughout development lifecycle
C) No security
D) Testing only

✔ Correct Answer: B
Reason: Secure SDLC incorporates security practices at every development phase.
Tag: Normal

---

### Question 10
Domain: Cyber Security
Topic: Wireless & Mobile Security
Subtopic: WiFi Security
Difficulty: Easy

Question: Which is the most secure WiFi encryption?
A) WEP
B) WPA
C) WPA3
D) Open network

✔ Correct Answer: C
Reason: WPA3 is latest and most secure WiFi encryption standard.
Tag: Normal

---

### Question 11
Domain: Cyber Security
Topic: Cloud & Virtualization Security
Subtopic: Cloud Security
Difficulty: Medium

Question: What is a shared responsibility model in cloud?
A) Provider handles everything
B) Security responsibilities split between provider and customer
C) Customer handles everything
D) No responsibility

✔ Correct Answer: B
Reason: Cloud security is shared between provider (infrastructure) and customer (data/apps).
Tag: Normal

---

### Question 12
Domain: Cyber Security
Topic: Digital Forensics
Subtopic: Evidence Collection
Difficulty: Medium

Question: What is chain of custody?
A) Random handling
B) Documented evidence handling process
C) No documentation
D) Evidence destruction

✔ Correct Answer: B
Reason: Chain of custody tracks evidence from collection to presentation.
Tag: Normal

---

### Question 13
Domain: Cyber Security
Topic: Incident Response & Management
Subtopic: Incident Response Phases
Difficulty: Easy

Question: What is the first phase of incident response?
A) Recovery
B) Preparation
C) Eradication
D) Lessons learned

✔ Correct Answer: B
Reason: Preparation involves establishing policies, tools, and training before incidents.
Tag: Normal

---

### Question 14
Domain: Cyber Security
Topic: Security Monitoring & Auditing
Subtopic: SIEM
Difficulty: Medium

Question: What does SIEM stand for?
A) Security Information Event Manager
B) Security Information and Event Management
C) System Internet Event Monitoring
D) Secure Internet Event Manager

✔ Correct Answer: B
Reason: SIEM aggregates and analyzes security data from multiple sources.
Tag: Normal

---

### Question 15
Domain: Cyber Security
Topic: Cyber Laws & Ethics
Subtopic: Legal Frameworks
Difficulty: Easy

Question: What is GDPR?
A) General Data Protection Regulation
B) Global Data Privacy Rule
C) Government Data Protection
D) General Digital Protocol

✔ Correct Answer: A
Reason: GDPR is EU regulation protecting personal data and privacy.
Tag: Normal

---

### Question 16
Domain: Cyber Security
Topic: Emerging Trends in Cyber Security
Subtopic: Zero Trust
Difficulty: Medium

Question: What is zero trust security model?
A) Trust everyone
B) Never trust, always verify
C) Trust internal users
D) No verification

✔ Correct Answer: B
Reason: Zero trust assumes no implicit trust, requiring continuous verification.
Tag: Normal

---

### Question 17
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: Hashing
Difficulty: Easy

Question: What is a hash function?
A) Reversible encryption
B) One-way function producing fixed-size output
C) Two-way encryption
D) Random function

✔ Correct Answer: B
Reason: Hash functions create fixed-length digest from variable input, irreversible.
Tag: Past Paper

---

### Question 18
Domain: Cyber Security
Topic: Network Security
Subtopic: IDS vs IPS
Difficulty: Medium

Question: What is the difference between IDS and IPS?
A) No difference
B) IDS detects, IPS detects and prevents
C) IPS only detects
D) IDS prevents only

✔ Correct Answer: B
Reason: IDS monitors and alerts; IPS actively blocks threats.
Tag: Normal

---

### Question 19
Domain: Cyber Security
Topic: Web Application Security
Subtopic: XSS
Difficulty: Medium

Question: What is Cross-Site Scripting (XSS)?
A) Database attack
B) Injecting malicious scripts into web pages
C) Network attack
D) Physical attack

✔ Correct Answer: B
Reason: XSS injects client-side scripts to steal data or hijack sessions.
Tag: Normal

---

### Question 20
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: Ransomware
Difficulty: Easy

Question: What is ransomware?
A) Free software
B) Malware encrypting data for ransom
C) Antivirus
D) Backup tool

✔ Correct Answer: B
Reason: Ransomware encrypts victim's files and demands payment for decryption.
Tag: Normal

---

### Question 21
Domain: Cyber Security
Topic: Authentication & Access Control
Subtopic: Biometric Authentication
Difficulty: Easy

Question: Which is a biometric authentication method?
A) Password
B) Fingerprint
C) PIN
D) Security question

✔ Correct Answer: B
Reason: Biometrics use physical characteristics like fingerprints for authentication.
Tag: Normal

---

### Question 22
Domain: Cyber Security
Topic: Security Fundamentals & Principles
Subtopic: AAA Framework
Difficulty: Medium

Question: What does AAA stand for in security?
A) Access, Audit, Authorization
B) Authentication, Authorization, Accounting
C) Availability, Access, Audit
D) Authentication, Access, Availability

✔ Correct Answer: B
Reason: AAA framework manages who accesses what and tracks activities.
Tag: Past Paper

---

### Question 23
Domain: Cyber Security
Topic: Operating System Security
Subtopic: Patch Management
Difficulty: Easy

Question: Why is patch management important?
A) Increase vulnerabilities
B) Fix security vulnerabilities
C) Slow down system
D) No importance

✔ Correct Answer: B
Reason: Patches fix known vulnerabilities and security flaws.
Tag: Normal

---

### Question 24
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: Asymmetric Encryption
Difficulty: Medium

Question: What is public key cryptography?
A) Same key for both operations
B) Different keys for encryption and decryption
C) No keys
D) Single private key

✔ Correct Answer: B
Reason: Asymmetric encryption uses public key for encryption, private for decryption.
Tag: Past Paper

---

### Question 25
Domain: Cyber Security
Topic: Network Security
Subtopic: VPN
Difficulty: Easy

Question: What is a VPN?
A) Virus Protection Network
B) Virtual Private Network
C) Very Private Network
D) Verified Public Network

✔ Correct Answer: B
Reason: VPN creates secure encrypted tunnel over public networks.
Tag: Normal

---

### Question 26
Domain: Cyber Security
Topic: Web Application Security
Subtopic: CSRF
Difficulty: Hard

Question: What is Cross-Site Request Forgery (CSRF)?
A) SQL injection
B) Forcing user to execute unwanted actions
C) XSS attack
D) DDoS attack

✔ Correct Answer: B
Reason: CSRF tricks authenticated users into performing unintended actions.
Tag: Normal

---

### Question 27
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: Phishing
Difficulty: Easy

Question: What is phishing?
A) Network scanning
B) Fraudulent attempt to obtain sensitive information
C) Firewall bypass
D) Encryption method

✔ Correct Answer: B
Reason: Phishing uses deceptive emails/websites to steal credentials.
Tag: Normal

---

### Question 28
Domain: Cyber Security
Topic: Secure Software Development
Subtopic: Input Validation
Difficulty: Medium

Question: Why is input validation important?
A) Improve performance
B) Prevent injection attacks
C) Increase speed
D) No importance

✔ Correct Answer: B
Reason: Input validation prevents malicious data from exploiting vulnerabilities.
Tag: Normal

---

### Question 29
Domain: Cyber Security
Topic: Digital Forensics
Subtopic: Forensic Analysis
Difficulty: Medium

Question: What is volatile data?
A) Permanent data
B) Data lost when power is off
C) Encrypted data
D) Backup data

✔ Correct Answer: B
Reason: Volatile data (RAM, cache) disappears without power, must be collected first.
Tag: Normal

---

### Question 30
Domain: Cyber Security
Topic: Incident Response & Management
Subtopic: Containment
Difficulty: Easy

Question: What is the purpose of containment in incident response?
A) Delete evidence
B) Limit damage and prevent spread
C) Ignore incident
D) Restart systems

✔ Correct Answer: B
Reason: Containment isolates affected systems to prevent further damage.
Tag: Normal

---

### Question 31
Domain: Cyber Security
Topic: Wireless & Mobile Security
Subtopic: Mobile Threats
Difficulty: Easy

Question: What is mobile malware?
A) Desktop virus
B) Malicious software targeting mobile devices
C) Network protocol
D) Mobile app

✔ Correct Answer: B
Reason: Mobile malware specifically targets smartphones and tablets.
Tag: Normal

---

### Question 32
Domain: Cyber Security
Topic: Cloud & Virtualization Security
Subtopic: Virtualization Risks
Difficulty: Medium

Question: What is VM escape?
A) VM backup
B) Breaking out of VM to access host
C) VM migration
D) VM shutdown

✔ Correct Answer: B
Reason: VM escape allows attacker to break isolation and access hypervisor/host.
Tag: Normal

---

### Question 33
Domain: Cyber Security
Topic: Security Monitoring & Auditing
Subtopic: Log Management
Difficulty: Easy

Question: Why are security logs important?
A) Waste space
B) Track activities and detect incidents
C) Slow system
D) No importance

✔ Correct Answer: B
Reason: Logs provide audit trail for security analysis and incident investigation.
Tag: Normal

---

### Question 34
Domain: Cyber Security
Topic: Cyber Laws & Ethics
Subtopic: Computer Fraud
Difficulty: Easy

Question: What is the Computer Fraud and Abuse Act (CFAA)?
A) Antivirus software
B) US law against unauthorized computer access
C) Encryption standard
D) Network protocol

✔ Correct Answer: B
Reason: CFAA criminalizes unauthorized access to computer systems.
Tag: Normal

---

### Question 35
Domain: Cyber Security
Topic: Emerging Trends in Cyber Security
Subtopic: AI in Security
Difficulty: Medium

Question: How is AI used in cybersecurity?
A) No use
B) Threat detection and response automation
C) Only for attacks
D) Slowing systems

✔ Correct Answer: B
Reason: AI analyzes patterns, detects anomalies, and automates security responses.
Tag: Normal

---

### Question 36
Domain: Cyber Security
Topic: Introduction to Cyber Security
Subtopic: Threat vs Vulnerability
Difficulty: Easy

Question: What is a vulnerability?
A) Active attack
B) Weakness that can be exploited
C) Security control
D) Firewall rule

✔ Correct Answer: B
Reason: Vulnerability is flaw or weakness in system that can be exploited.
Tag: Past Paper

---

### Question 37
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: Digital Signatures
Difficulty: Medium

Question: What is a digital signature?
A) Handwritten signature
B) Cryptographic verification of authenticity
C) Password
D) Username

✔ Correct Answer: B
Reason: Digital signature uses asymmetric cryptography to verify sender and integrity.
Tag: Normal

---

### Question 38
Domain: Cyber Security
Topic: Network Security
Subtopic: DMZ
Difficulty: Medium

Question: What is a DMZ in network security?
A) Demilitarized Zone - isolated network segment
B) Data Management Zone
C) Digital Memory Zone
D) Direct Memory Zone

✔ Correct Answer: A
Reason: DMZ is buffer network between internal network and internet.
Tag: Normal

---

### Question 39
Domain: Cyber Security
Topic: Web Application Security
Subtopic: HTTPS
Difficulty: Easy

Question: What does HTTPS provide?
A) Faster speed
B) Encrypted communication
C) More storage
D) Better graphics

✔ Correct Answer: B
Reason: HTTPS uses SSL/TLS to encrypt data between browser and server.
Tag: Normal

---

### Question 40
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: Trojan Horse
Difficulty: Easy

Question: What is a Trojan horse?
A) Antivirus
B) Malware disguised as legitimate software
C) Firewall
D) Encryption tool

✔ Correct Answer: B
Reason: Trojan appears harmless but contains malicious code.
Tag: Normal

---

### Question 41
Domain: Cyber Security
Topic: Authentication & Access Control
Subtopic: Password Security
Difficulty: Easy

Question: What makes a strong password?
A) Short and simple
B) Long with mixed characters
C) Dictionary word
D) Username

✔ Correct Answer: B
Reason: Strong passwords use length, uppercase, lowercase, numbers, and symbols.
Tag: Normal

---

### Question 42
Domain: Cyber Security
Topic: Security Fundamentals & Principles
Subtopic: Risk Management
Difficulty: Medium

Question: What is risk assessment?
A) Ignoring risks
B) Identifying and evaluating security risks
C) Accepting all risks
D) No evaluation

✔ Correct Answer: B
Reason: Risk assessment identifies threats, vulnerabilities, and potential impacts.
Tag: Normal

---

### Question 43
Domain: Cyber Security
Topic: Operating System Security
Subtopic: File Permissions
Difficulty: Easy

Question: In Unix/Linux, what does chmod 755 mean?
A) Owner: rwx, Group: r-x, Others: r-x
B) All: full access
C) No access
D) Owner only: read

✔ Correct Answer: A
Reason: 755 = owner (7=rwx), group (5=r-x), others (5=r-x).
Tag: Normal

---

### Question 44
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: SSL/TLS
Difficulty: Medium

Question: What is TLS?
A) Total Layer Security
B) Transport Layer Security
C) Transfer Layer System
D) Technical Layer Security

✔ Correct Answer: B
Reason: TLS is cryptographic protocol providing secure communication over networks.
Tag: Normal

---

### Question 45
Domain: Cyber Security
Topic: Network Security
Subtopic: Port Scanning
Difficulty: Medium

Question: What is port scanning?
A) Physical inspection
B) Probing network for open ports
C) Closing ports
D) Port forwarding

✔ Correct Answer: B
Reason: Port scanning identifies open ports and services on target system.
Tag: Normal

---

### Question 46
Domain: Cyber Security
Topic: Web Application Security
Subtopic: Security Headers
Difficulty: Hard

Question: What does Content-Security-Policy header do?
A) Compress content
B) Prevent XSS and injection attacks
C) Cache content
D) No function

✔ Correct Answer: B
Reason: CSP header restricts resource sources to prevent XSS attacks.
Tag: Normal

---

### Question 47
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: DDoS
Difficulty: Medium

Question: What is a DDoS attack?
A) Data encryption
B) Distributed Denial of Service attack
C) Database optimization
D) Data backup

✔ Correct Answer: B
Reason: DDoS overwhelms target with traffic from multiple sources.
Tag: Past Paper

---

### Question 48
Domain: Cyber Security
Topic: Secure Software Development
Subtopic: Code Review
Difficulty: Medium

Question: Why conduct security code reviews?
A) Slow development
B) Identify vulnerabilities before deployment
C) No benefit
D) Increase bugs

✔ Correct Answer: B
Reason: Code reviews catch security flaws early in development.
Tag: Normal

---

### Question 49
Domain: Cyber Security
Topic: Digital Forensics
Subtopic: Disk Imaging
Difficulty: Medium

Question: What is forensic disk imaging?
A) Deleting data
B) Creating exact copy of storage media
C) Formatting disk
D) Compressing files

✔ Correct Answer: B
Reason: Disk imaging creates bit-by-bit copy for analysis without altering original.
Tag: Normal

---

### Question 50
Domain: Cyber Security
Topic: Incident Response & Management
Subtopic: Eradication
Difficulty: Easy

Question: What happens during eradication phase?
A) Monitoring only
B) Removing threat from environment
C) Documentation only
D) No action

✔ Correct Answer: B
Reason: Eradication eliminates malware, closes vulnerabilities, and removes attacker access.
Tag: Normal

---

### Question 51
Domain: Cyber Security
Topic: Wireless & Mobile Security
Subtopic: Bluetooth Security
Difficulty: Medium

Question: What is bluejacking?
A) Bluetooth hacking for data theft
B) Sending unsolicited messages via Bluetooth
C) Bluetooth encryption
D) Bluetooth pairing

✔ Correct Answer: B
Reason: Bluejacking sends anonymous messages to nearby Bluetooth devices.
Tag: Normal

---

### Question 52
Domain: Cyber Security
Topic: Cloud & Virtualization Security
Subtopic: Container Security
Difficulty: Hard

Question: What is container escape?
A) Container backup
B) Breaking out of container to access host
C) Container migration
D) Container shutdown

✔ Correct Answer: B
Reason: Container escape exploits vulnerabilities to access underlying host system.
Tag: Normal

---

### Question 53
Domain: Cyber Security
Topic: Security Monitoring & Auditing
Subtopic: Vulnerability Scanning
Difficulty: Easy

Question: What is vulnerability scanning?
A) Creating vulnerabilities
B) Automated identification of security weaknesses
C) Exploiting vulnerabilities
D) Ignoring vulnerabilities

✔ Correct Answer: B
Reason: Vulnerability scanners automatically detect known security flaws.
Tag: Normal

---

### Question 54
Domain: Cyber Security
Topic: Cyber Laws & Ethics
Subtopic: Ethical Hacking
Difficulty: Easy

Question: What is ethical hacking?
A) Illegal hacking
B) Authorized security testing
C) Malicious hacking
D) No hacking

✔ Correct Answer: B
Reason: Ethical hackers test systems with permission to find vulnerabilities.
Tag: Normal

---

### Question 55
Domain: Cyber Security
Topic: Emerging Trends in Cyber Security
Subtopic: Blockchain Security
Difficulty: Hard

Question: How does blockchain enhance security?
A) No security
B) Immutable distributed ledger
C) Centralized control
D) Easy modification

✔ Correct Answer: B
Reason: Blockchain's distributed, tamper-resistant nature enhances data integrity.
Tag: Normal

---

### Question 56
Domain: Cyber Security
Topic: Introduction to Cyber Security
Subtopic: Security Threats
Difficulty: Easy

Question: What is a threat actor?
A) Security tool
B) Entity that poses security threat
C) Firewall
D) Antivirus

✔ Correct Answer: B
Reason: Threat actor is individual or group that carries out attacks.
Tag: Normal

---

### Question 57
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: AES
Difficulty: Medium

Question: What is AES?
A) Advanced Encryption Standard
B) Automatic Encryption System
C) Applied Encryption Security
D) Advanced Electronic Security

✔ Correct Answer: A
Reason: AES is widely-used symmetric encryption algorithm.
Tag: Past Paper

---

### Question 58
Domain: Cyber Security
Topic: Network Security
Subtopic: NAT
Difficulty: Medium

Question: What security benefit does NAT provide?
A) No benefit
B) Hides internal IP addresses
C) Increases speed
D) Encrypts data

✔ Correct Answer: B
Reason: NAT obscures internal network structure from external networks.
Tag: Normal

---

### Question 59
Domain: Cyber Security
Topic: Web Application Security
Subtopic: Session Management
Difficulty: Medium

Question: What is session hijacking?
A) Creating sessions
B) Stealing user session to impersonate
C) Ending sessions
D) Session encryption

✔ Correct Answer: B
Reason: Session hijacking captures session tokens to gain unauthorized access.
Tag: Normal

---

### Question 60
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: Rootkit
Difficulty: Hard

Question: What is a rootkit?
A) Root password
B) Malware hiding its presence
C) Admin tool
D) Backup system

✔ Correct Answer: B
Reason: Rootkit conceals malware by modifying OS to avoid detection.
Tag: Normal

---

### Question 61
Domain: Cyber Security
Topic: Authentication & Access Control
Subtopic: Single Sign-On
Difficulty: Medium

Question: What is SSO?
A) Single Security Option
B) Single Sign-On for multiple applications
C) Super Secure Operation
D) System Security Only

✔ Correct Answer: B
Reason: SSO allows one authentication for access to multiple systems.
Tag: Normal

---

### Question 62
Domain: Cyber Security
Topic: Security Fundamentals & Principles
Subtopic: Security by Obscurity
Difficulty: Medium

Question: Why is security by obscurity discouraged?
A) Very effective
B) Relies on secrecy, not strong design
C) Best practice
D) Always works

✔ Correct Answer: B
Reason: Security by obscurity fails when secret is discovered; use proven methods.
Tag: Normal

---

### Question 63
Domain: Cyber Security
Topic: Operating System Security
Subtopic: SELinux
Difficulty: Hard

Question: What is SELinux?
A) Linux distribution
B) Security-Enhanced Linux with mandatory access control
C) Linux firewall
D) Linux antivirus

✔ Correct Answer: B
Reason: SELinux implements MAC (Mandatory Access Control) in Linux kernel.
Tag: Normal

---

### Question 64
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: PKI
Difficulty: Medium

Question: What is PKI?
A) Private Key Infrastructure
B) Public Key Infrastructure
C) Protected Key Internet
D) Public Kernel Interface

✔ Correct Answer: B
Reason: PKI manages digital certificates and public-key encryption.
Tag: Normal

---

### Question 65
Domain: Cyber Security
Topic: Network Security
Subtopic: Honeypot
Difficulty: Medium

Question: What is a honeypot?
A) Sweet storage
B) Decoy system to attract attackers
C) Backup server
D) Load balancer

✔ Correct Answer: B
Reason: Honeypot lures attackers to study tactics and protect real systems.
Tag: Normal

---

### Question 66
Domain: Cyber Security
Topic: Web Application Security
Subtopic: OWASP Top 10
Difficulty: Easy

Question: What is OWASP?
A) Operating Web Application Security Protocol
B) Open Web Application Security Project
C) Online Web Access Security Platform
D) Organized Web Application System

✔ Correct Answer: B
Reason: OWASP is nonprofit focused on improving software security.
Tag: Normal

---

### Question 67
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: Social Engineering
Difficulty: Easy

Question: What is social engineering?
A) Network attack
B) Manipulating people to divulge information
C) Software vulnerability
D) Encryption method

✔ Correct Answer: B
Reason: Social engineering exploits human psychology rather than technical vulnerabilities.
Tag: Past Paper

---

### Question 68
Domain: Cyber Security
Topic: Secure Software Development
Subtopic: OWASP SAMM
Difficulty: Hard

Question: What is OWASP SAMM?
A) Security tool
B) Software Assurance Maturity Model
C) Malware scanner
D) Network protocol

✔ Correct Answer: B
Reason: SAMM helps organizations assess and improve software security practices.
Tag: Normal

---

### Question 69
Domain: Cyber Security
Topic: Digital Forensics
Subtopic: Network Forensics
Difficulty: Medium

Question: What is network forensics?
A) Network design
B) Capturing and analyzing network traffic
C) Network speed testing
D) Network installation

✔ Correct Answer: B
Reason: Network forensics monitors and analyzes network data for investigations.
Tag: Normal

---

### Question 70
Domain: Cyber Security
Topic: Incident Response & Management
Subtopic: Recovery
Difficulty: Easy

Question: What happens during recovery phase?
A) Deleting systems
B) Restoring systems to normal operation
C) Ignoring incident
D) No action

✔ Correct Answer: B
Reason: Recovery restores affected systems and services to operational state.
Tag: Normal

---

### Question 71
Domain: Cyber Security
Topic: Wireless & Mobile Security
Subtopic: BYOD
Difficulty: Medium

Question: What security challenge does BYOD present?
A) No challenges
B) Managing personal devices accessing corporate data
C) Faster networks
D) Better security

✔ Correct Answer: B
Reason: BYOD (Bring Your Own Device) creates security and policy enforcement challenges.
Tag: Normal

---

### Question 72
Domain: Cyber Security
Topic: Cloud & Virtualization Security
Subtopic: Multi-tenancy
Difficulty: Medium

Question: What is multi-tenancy risk in cloud?
A) No risk
B) Data isolation between customers
C) Faster performance
D) Lower cost

✔ Correct Answer: B
Reason: Multi-tenancy requires strong isolation to prevent data leakage between tenants.
Tag: Normal

---

### Question 73
Domain: Cyber Security
Topic: Security Monitoring & Auditing
Subtopic: Penetration Testing
Difficulty: Medium

Question: What is penetration testing?
A) Network installation
B) Simulated attack to find vulnerabilities
C) Software development
D) Data backup

✔ Correct Answer: B
Reason: Pen testing ethically exploits systems to identify security weaknesses.
Tag: Normal

---

### Question 74
Domain: Cyber Security
Topic: Cyber Laws & Ethics
Subtopic: Data Breach Notification
Difficulty: Easy

Question: Why are data breach notifications required?
A) No reason
B) Inform affected individuals of compromise
C) Hide breaches
D) Slow response

✔ Correct Answer: B
Reason: Breach notification laws require timely disclosure to affected parties.
Tag: Normal

---

### Question 75
Domain: Cyber Security
Topic: Emerging Trends in Cyber Security
Subtopic: Quantum Cryptography
Difficulty: Hard

Question: What threat does quantum computing pose to cryptography?
A) No threat
B) Breaking current encryption algorithms
C) Stronger encryption
D) Faster encryption

✔ Correct Answer: B
Reason: Quantum computers could break RSA and other current encryption methods.
Tag: Normal

---

### Question 76
Domain: Cyber Security
Topic: Introduction to Cyber Security
Subtopic: Attack Surface
Difficulty: Easy

Question: What is attack surface?
A) Physical area
B) Sum of vulnerabilities accessible to attackers
C) Network speed
D) Storage capacity

✔ Correct Answer: B
Reason: Attack surface includes all points where unauthorized user can enter or extract data.
Tag: Normal

---

### Question 77
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: Salt
Difficulty: Medium

Question: What is salt in password hashing?
A) Seasoning
B) Random data added before hashing
C) Encryption key
D) Hash algorithm

✔ Correct Answer: B
Reason: Salt adds randomness to prevent rainbow table attacks on hashed passwords.
Tag: Normal

---

### Question 78
Domain: Cyber Security
Topic: Network Security
Subtopic: MAC Filtering
Difficulty: Easy

Question: What is MAC address filtering?
A) Computer brand filtering
B) Allowing/blocking devices by hardware address
C) IP filtering
D) Port filtering

✔ Correct Answer: B
Reason: MAC filtering controls network access based on device MAC addresses.
Tag: Normal

---

### Question 79
Domain: Cyber Security
Topic: Web Application Security
Subtopic: API Security
Difficulty: Medium

Question: What is API rate limiting?
A) Slowing APIs
B) Restricting number of requests per time period
C) Stopping APIs
D) No limits

✔ Correct Answer: B
Reason: Rate limiting prevents abuse and DoS attacks on APIs.
Tag: Normal

---

### Question 80
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: Zero-Day Exploit
Difficulty: Hard

Question: What is a zero-day exploit?
A) Old vulnerability
B) Attack on unknown vulnerability
C) Patched vulnerability
D) No vulnerability

✔ Correct Answer: B
Reason: Zero-day exploits target vulnerabilities before vendor awareness or patches.
Tag: Normal

---

### Question 81
Domain: Cyber Security
Topic: Authentication & Access Control
Subtopic: OAuth
Difficulty: Medium

Question: What is OAuth?
A) Password protocol
B) Open standard for access delegation
C) Encryption method
D) Firewall rule

✔ Correct Answer: B
Reason: OAuth allows third-party access without sharing credentials.
Tag: Normal

---

### Question 82
Domain: Cyber Security
Topic: Security Fundamentals & Principles
Subtopic: Separation of Duties
Difficulty: Medium

Question: What is separation of duties?
A) Single person control
B) Dividing tasks among multiple people
C) No division
D) Random assignment

✔ Correct Answer: B
Reason: Separation of duties prevents fraud by requiring multiple parties for critical tasks.
Tag: Normal

---

### Question 83
Domain: Cyber Security
Topic: Operating System Security
Subtopic: Secure Boot
Difficulty: Medium

Question: What is secure boot?
A) Fast boot
B) Verifying boot components before loading
C) No verification
D) Random boot

✔ Correct Answer: B
Reason: Secure boot ensures only trusted software loads during startup.
Tag: Normal

---

### Question 84
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: Certificate Authority
Difficulty: Easy

Question: What is a Certificate Authority (CA)?
A) Government agency
B) Trusted entity issuing digital certificates
C) Antivirus company
D) ISP

✔ Correct Answer: B
Reason: CA verifies identity and issues digital certificates for secure communication.
Tag: Normal

---

### Question 85
Domain: Cyber Security
Topic: Network Security
Subtopic: VLAN
Difficulty: Medium

Question: How do VLANs improve security?
A) No improvement
B) Segment network to isolate traffic
C) Increase speed
D) Reduce cost

✔ Correct Answer: B
Reason: VLANs create logical network segments to contain broadcast and limit access.
Tag: Normal

---

### Question 86
Domain: Cyber Security
Topic: Web Application Security
Subtopic: Input Sanitization
Difficulty: Easy

Question: What is input sanitization?
A) Accepting all input
B) Cleaning user input to prevent attacks
C) Ignoring input
D) No validation

✔ Correct Answer: B
Reason: Sanitization removes or escapes dangerous characters from user input.
Tag: Normal

---

### Question 87
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: Spyware
Difficulty: Easy

Question: What is spyware?
A) Antivirus
B) Software secretly monitoring user activity
C) Firewall
D) Encryption tool

✔ Correct Answer: B
Reason: Spyware collects information without user knowledge or consent.
Tag: Normal

---

### Question 88
Domain: Cyber Security
Topic: Secure Software Development
Subtopic: Threat Modeling
Difficulty: Medium

Question: What is threat modeling?
A) Random analysis
B) Identifying and prioritizing potential threats
C) No analysis
D) Performance testing

✔ Correct Answer: B
Reason: Threat modeling systematically identifies security threats during design.
Tag: Normal

---

### Question 89
Domain: Cyber Security
Topic: Digital Forensics
Subtopic: Memory Forensics
Difficulty: Hard

Question: What is memory forensics?
A) Disk analysis
B) Analyzing RAM for evidence
C) Network analysis
D) File analysis

✔ Correct Answer: B
Reason: Memory forensics examines volatile memory for malware, passwords, and artifacts.
Tag: Normal

---

### Question 90
Domain: Cyber Security
Topic: Incident Response & Management
Subtopic: Lessons Learned
Difficulty: Easy

Question: What is the purpose of lessons learned phase?
A) Blame individuals
B) Improve future incident response
C) Ignore incident
D) No purpose

✔ Correct Answer: B
Reason: Lessons learned reviews incident to improve processes and prevent recurrence.
Tag: Normal

---

### Question 91
Domain: Cyber Security
Topic: Wireless & Mobile Security
Subtopic: Evil Twin Attack
Difficulty: Medium

Question: What is an evil twin attack?
A) Legitimate access point
B) Rogue access point mimicking legitimate one
C) Firewall
D) Encryption method

✔ Correct Answer: B
Reason: Evil twin is fake WiFi hotspot impersonating legitimate network.
Tag: Normal

---

### Question 92
Domain: Cyber Security
Topic: Cloud & Virtualization Security
Subtopic: Data Encryption in Cloud
Difficulty: Easy

Question: Why encrypt data in cloud?
A) Slow access
B) Protect confidentiality
C) Increase cost
D) No reason

✔ Correct Answer: B
Reason: Encryption protects data from unauthorized access in cloud environments.
Tag: Normal

---

### Question 93
Domain: Cyber Security
Topic: Security Monitoring & Auditing
Subtopic: Security Baseline
Difficulty: Medium

Question: What is a security baseline?
A) Random configuration
B) Minimum security standards
C) Maximum security
D) No standards

✔ Correct Answer: B
Reason: Security baseline defines minimum acceptable security configuration.
Tag: Normal

---

### Question 94
Domain: Cyber Security
Topic: Cyber Laws & Ethics
Subtopic: Intellectual Property
Difficulty: Easy

Question: What is software piracy?
A) Legal copying
B) Unauthorized copying or distribution
C) Purchasing software
D) Open source

✔ Correct Answer: B
Reason: Software piracy violates copyright by illegal copying or distribution.
Tag: Normal

---

### Question 95
Domain: Cyber Security
Topic: Emerging Trends in Cyber Security
Subtopic: IoT Security
Difficulty: Medium

Question: What is a major IoT security challenge?
A) Too secure
B) Many devices with weak security
C) No devices
D) Perfect security

✔ Correct Answer: B
Reason: IoT devices often lack security features and create large attack surface.
Tag: Normal

---

### Question 96
Domain: Cyber Security
Topic: Introduction to Cyber Security
Subtopic: Security Posture
Difficulty: Easy

Question: What is security posture?
A) Physical stance
B) Overall security status of organization
C) Network speed
D) Storage capacity

✔ Correct Answer: B
Reason: Security posture reflects organization's cybersecurity strength and readiness.
Tag: Normal

---

### Question 97
Domain: Cyber Security
Topic: Cryptography Basics
Subtopic: Key Management
Difficulty: Medium

Question: Why is key management important?
A) No importance
B) Secure generation, storage, and distribution of keys
C) Slow encryption
D) Increase cost

✔ Correct Answer: B
Reason: Poor key management undermines encryption effectiveness.
Tag: Normal

---

### Question 98
Domain: Cyber Security
Topic: Network Security
Subtopic: Network Segmentation
Difficulty: Medium

Question: What is network segmentation?
A) Combining networks
B) Dividing network into isolated segments
C) No division
D) Single network

✔ Correct Answer: B
Reason: Segmentation limits lateral movement and contains breaches.
Tag: Normal

---

### Question 99
Domain: Cyber Security
Topic: Web Application Security
Subtopic: Security Misconfiguration
Difficulty: Easy

Question: What is security misconfiguration?
A) Perfect configuration
B) Insecure default or incomplete settings
C) No configuration
D) Optimal settings

✔ Correct Answer: B
Reason: Misconfiguration leaves systems vulnerable through improper settings.
Tag: Normal

---

### Question 100
Domain: Cyber Security
Topic: Malware & Attack Techniques
Subtopic: APT
Difficulty: Hard

Question: What is an Advanced Persistent Threat (APT)?
A) Simple attack
B) Prolonged targeted attack by sophisticated actors
C) One-time attack
D) Automated attack

✔ Correct Answer: B
Reason: APT is stealthy, continuous attack campaign by skilled adversaries.
Tag: Normal

---

## End of Batch 01

---
